...
TEST_CASE("Check if a generated capped cylinder is not detected as sphere", "[TriMesh]")
{
    constexpr float Radius = 4;
    TriMesh mesh = TriMesh::MakeCylinder(Vector3f::Zero(), Radius, Vector3f::UnitZ(), Radius, 32, true);

    auto sphere = mesh.getAsSphere();
    REQUIRE_FALSE(sphere.has_value());
}

TEST_CASE("Check if a generated non-capped cylinder is not detected as sphere", "[TriMesh]")
{
    constexpr float Radius = 4;
    TriMesh mesh = TriMesh::MakeCylinder(Vector3f::Zero(), Radius, Vector3f::UnitZ(), Radius, 32, false);
