...
def test_point_light():
    ...
    scene = create_flat_scene()
    scene["lights"].append(
        {"type": "point", "name": "_light", "position": [0, 0, -2], "power": 1})
    value = compute_scene_average(scene)
    assert value == pytest.approx(0.005100456, abs=1e-4)
...
def test_spot_light():
    ...
    # Therefore, the actual scene average is 0.005100456 * 6.82842712474 = 0.0348280902
    scene = create_flat_scene()
    scene["lights"].append(
        {"type": "spot", "name": "_light", "cutoff": 45, "falloff": 45, "position": [0, 0, -2], "direction": [0, 0, 1], "power": 1})
    value = compute_scene_average(scene)
    assert value == pytest.approx(0.0348280902, abs=2.5e-3) # TODO: Some tiny error is still apparant. Investigate!
