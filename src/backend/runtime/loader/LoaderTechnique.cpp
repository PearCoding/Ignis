...
static AOVInfo technique_empty_get_aov_info(const std::string&, const std::shared_ptr<Parser::Object>&, const LoaderContext&)
{
	return {};
}
...
static AOVInfo path_get_aov_info(const std::string&, const std::shared_ptr<Parser::Object>& technique, const LoaderContext&)
{
	AOVInfo info;
...
static void path_body_loader(std::ostream& stream, const std::string&, const std::shared_ptr<Parser::Object>& technique, const LoaderContext&)
{
	...
	if (hasMISAOV) {
		stream << "  let aov_di = device.load_aov_image(AOV_PATH_DIRECT, spp);" << std::endl
			   << "  let aov_nee = device.load_aov_image(AOV_PATH_NEE, spp);" << std::endl;
	}
...
using TechniqueGetAOVInfo = AOVInfo (*)(const std::string&, const std::shared_ptr<Parser::Object>&, const LoaderContext&);
...
static struct TechniqueEntry {
	const char* Name;
	TechniqueGetAOVInfo GetAOVInfo;
	TechniqueBodyLoader BodyLoader;
	TechniqueHeaderLoader HeaderLoader;
} _generators[] = {
	{ "ao", technique_empty_get_aov_info, ao_body_loader, technique_empty_header_loader },
	{ "path", path_get_aov_info, path_body_loader, path_header_loader },
	{ "debug", technique_empty_get_aov_info, debug_body_loader, technique_empty_header_loader },
...
AOVInfo LoaderTechnique::getAOVInfo(const LoaderContext& ctx)
{
	...
	return entry->GetAOVInfo(ctx.TechniqueType, technique, ctx);
}
