...
TechniqueInfo LightVisibilityTechnique::getInfo(const LoaderContext&) const
{
    ...
    info.PrimaryPayloadCount          = 1;
    info.SecondaryPayloadCount        = 1;
    info.EmitterPayloadInitializer    = "make_simple_payload_initializer(init_lv_raypayload)";
    info.Passes[0].ShadowHandlingMode = ShadowHandlingMode::AdvancedWithMaterials;
    return info;
}
