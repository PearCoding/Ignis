...
void DJMeasuredBSDF::serialize(const SerializationInput& input) const
{
    ...
    input.Stream << input.Tree.pullHeader()
                 << "  let " << buffer_name << "_ndf : DeviceBuffer = device.load_buffer(\"" << out_path.generic_string() << "_ndf\");\n"
                 << "  let " << buffer_name << "_vndf : DeviceBuffer = device.load_buffer(\"" << out_path.generic_string() << "_vndf\");\n"
                 << "  let " << buffer_name << "_sigma : DeviceBuffer = device.load_buffer(\"" << out_path.generic_string() << "_sigma\");\n"
                 << "  let " << buffer_name << "_luminance : DeviceBuffer = device.load_buffer(\"" << out_path.generic_string() << "_luminance\");\n"
                 << "  let " << buffer_name << "_rgb : DeviceBuffer = device.load_buffer(\"" << out_path.generic_string() << "_rgb\");\n"
