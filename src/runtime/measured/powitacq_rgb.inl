...
public:
    ...
    Marginal2D(const Vector2u& size, const float* data,
        ...
        : m_size(size)
        , m_patch_size(Vector2f(1.f) / Vector2f(m_size - 1u))
        ...
        if (build_cdf) {
            ...
            for (uint32_t slice = 0; slice < slices; ++slice) {
                ...
                for (uint32_t y = 0; y < m_size.y() - 1; ++y) {
                    sum += .5 * ((double)conditional_cdf[(y + 1) * size.x() - 1] + (double)conditional_cdf[(y + 2) * size.x() - 1]);
                                      ...
                                      const float* param = nullptr) const
    {
        ...
        sample.y()    = is_const ? (2.f * sample.y()) : (r0 - std::sqrt(r0 * r0 - 2.f * sample.y() * (r0 - r1)));
        ...
        sample.x() = is_const ? (2.f * sample.x()) : (c0 - std::sqrt(c0 * c0 - 2.f * sample.x() * (c0 - c1)));
    ...
    {
        /* Look up parameter-related indices and weights (if Dimension != 0) */
        std::array<float, 2 * ArraySize> param_weight;
        uint32_t slice_offset = 0u;
...
public:
    ...
    std::array<uint32_t, ArraySize> m_param_size;

    /// Stride per parameter in units of sizeof(float)
    std::array<uint32_t, ArraySize> m_param_strides;

    /// Discretization of each parameter domain
    std::array<FloatStorage, ArraySize> m_param_values;
...
template <typename Value>
Value theta2u(Value theta)
{
    return std::sqrt(theta * (2.f / Pi));
}

template <typename Value>
Value phi2u(Value phi)
{
...
BRDF::BRDF(const std::string& path_to_file)
{
    ...
    if (!(description.shape.size() == 1 && description.dtype == Tensor::UInt8 &&
          ...
          rgb.dtype == Tensor::Float32 && rgb.shape.size() == 5 && rgb.shape[0] == phi_i.shape[0] && rgb.shape[1] == theta_i.shape[0] && rgb.shape[2] == 3 && rgb.shape[3] == luminance.shape[2] &&

          luminance.shape[2] == rgb.shape[3] && luminance.shape[3] == rgb.shape[4] &&
...
float BRDF::pdf(const Vector3f& wi, const Vector3f& wo) const
{
    ...
    float jacobian    = std::max(2.f * sqr(Pi) * u_wm.x() * sin_theta_m, 1e-6f) * 4.f * dot(wi, wm);
...
Vector3f BRDF::eval(const Vector3f& wi, const Vector3f& wo) const
{
    ...
    fr = fr * m_data->ndf.eval(u_wm, params) / (4 * m_data->sigma.eval(u_wi, params));
...
#if POWITACQ_SAMPLE_LUMINANCE
    std::tie(sample, lum_pdf) = m_data->luminance.sample(sample, params);
#endif

    Vector2f u_wm;
    float ndf_pdf;
    std::tie(u_wm, ndf_pdf) = m_data->vndf.sample(sample, params);
