...
public:
    ...
    [[nodiscard]] static std::vector<int> triangulate(const std::vector<Vector3f>& vertices);

    /// Triangulate a two dimensional polygon and return a list of indices
    [[nodiscard]] static std::vector<int> triangulate(const std::vector<Vector2f>& vertices);
};
} // namespace IG